...
export default function System_auth() {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/GetTables?keySpace=system_auth')
            .then(
                (result) => {
                    setIsLoaded(true);
    ...
    useEffect(() => {
        fetch('http://localhost:8080/GetTables?keySpace=system_auth')
